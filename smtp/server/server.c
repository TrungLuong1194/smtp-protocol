			...
			if (pollfds[i].revents & POLLIN) {
				...
				/* ---------- WAIT STATE ---------- */
					...
					if (is_matching_pattern(response, HELO_CMD) == TRUE) {
						...
						current_state[i] = ENVELOPE_CREATED_STATE;
					} else if (is_matching_pattern(response, RCPT_CMD) == TRUE) {
						msg = "503 Error: need MAIL command\n";
						send(pollfds[i].fd, msg, strlen(msg), 0);
					} else if (is_matching_pattern(response, DATA_CMD) == TRUE) {
				...
				/* ---------- ENVELOPE CREATED STATE ---------- */
				...
				else if (current_state[i] == ENVELOPE_CREATED_STATE) {
					...
					if (is_matching_pattern(response, HELO_CMD) == TRUE) {
						...
						current_state[i] = RECIPIENTS_SET_STATE;
					} else if (is_matching_pattern(response, DATA_CMD) == TRUE) {
						msg = "503 Error: need RCPT command\n";
						send(pollfds[i].fd, msg, strlen(msg), 0);
					} else if (is_matching_pattern(response, RSET_CMD) == TRUE) {
				...
				/* ---------- RECIPIENTS SET STATE ---------- */

				else if (current_state[i] == RECIPIENTS_SET_STATE) {
					...
					if (is_matching_pattern(response, HELO_CMD) == TRUE) {
						...
						current_state[i] = WRITING_MAIL_STATE;
					} else if (is_matching_pattern(response, RSET_CMD) == TRUE) {
						msg = "250 OK\n";
						send(pollfds[i].fd, msg, strlen(msg), 0);
				...
				else if (current_state[i] == WRITING_MAIL_STATE) {
					if (strcmp(response, ".\n") != 0) {
						...
						current_state[i] = READY_TO_DELIVER_STATE;
					}
				}

				/* ---------- READY TO DELIVER STATE (Back to WAIT STATE) ---------- */

				else if (current_state[i] == READY_TO_DELIVER_STATE) {
